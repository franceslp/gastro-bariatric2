...
COHORTS = {"with_BMI_optimal": "a1c_trajectory_with_BMI_optimal.csv"}
...
try:
    ...
    if "age_at_surgery_approx" in mat.columns:
        ...
        if char_rows:
            ...
            cdf.to_csv("baseline_characteristics_by_surgery_optimal.csv", index=False)
            report.append("\n" + "="*66)
            report.append("BASELINE CHARACTERISTICS BY SURGERY TYPE (full covariate matrix)")
            report.append("="*66)
            report.append(cdf.to_string(index=False))
            report.append("  (saved to baseline_characteristics_by_surgery_optimal.csv)")
except Exception as e:
    report.append(f"\nBaseline characteristics table skipped: {e}")

pd.DataFrame(A_rows).to_csv("a1c_surgtype_sleeve_vs_bypass_optimal.csv", index=False)
pd.DataFrame(B_rows).to_csv("a1c_surgtype_gp_vs_comp_stratified_optimal.csv", index=False)
pd.DataFrame(cell_rows).to_csv("a1c_surgtype_cellcounts_optimal.csv", index=False)

text = "\n".join(report)
with open("a1c_surgtype_summary_optimal.txt", "w") as f:
    f.write(text)
print(text)
