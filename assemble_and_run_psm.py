...
print(">>> SCRIPT VERSION: assemble_and_run_psm_v1 <<<")
...
smd_before.to_csv("psm_smd_before_matching.csv", index=False)
...
# ===========================================================================
psm_df.to_csv("psm_full_covariate_matrix.csv", index=False)
pairs_df.to_csv("psm_matched_pairs_new.csv", index=False)
balance.to_csv("psm_balance_table_new.csv", index=False)
smd_before.to_csv("psm_smd_before_matching.csv", index=False)
smd_after.to_csv("psm_smd_after_matching.csv", index=False)

# Full matched dataset
matched_dataset = psm_df[
    psm_df["patient_id"].isin(matched_gp_ids | matched_comp_ids)
].copy()
matched_dataset.to_csv("psm_matched_dataset_new.csv", index=False)
